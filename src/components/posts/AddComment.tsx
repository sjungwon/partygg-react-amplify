...
export default function AddComment({
  ...
  //유저 데이터 사용 -> 프로필, 이름
  const { username, currentProfile: defaultProfile } =
    useContext(UserDataContext);

  const [currentProfile, setCurrentProfile] = useState<Profile>(defaultProfile);

  useEffect(() => {
    //수정인 경우 수정할 profile로 설정
    if (prevData) {
      setCurrentProfile(prevData.data.profile);
    }
    //수정이 아닌 경우
  ...
  const textFocusControl = useCallback(
    (e: any) =>
      (e.currentTarget.selectionStart = e.currentTarget.selectionEnd =
        prevData ? prevData.data.text.length : 0),
    [prevData]
  );
  ...
  return (
    <div className={styles.add_comment_border}>
      ...
      <textarea
        ...
        ref={textAreaRef}
        defaultValue={prevData ? prevData.data.text : ""}
        autoFocus
        onFocus={textFocusControl}
      />

      <button
        className={styles.add_comment_submit}
